...
query = """SELECT id FROM {0} WHERE name = %s;"""

query2 = """INSERT INTO {0} (name, eng_name) VALUES (%s, %s);"""
...
with open('staff.csv') as f:
    for person in f:
        ...
        url = info[3]
        print('Downloading {0} {1} ...'.format(info[0], info[1]))
        data = get_more_data_ignore_office_people(url)
        
        for name, eng_name in data['departments']:
	        cur.execute(query.format('department'), (name,))
	        if cur.fetchone() is None:
		        cur.execute(query2.format('department'), (name, eng_names))

        for name, eng_name in data['subjects']:
	        cur.execute(query.format('subject'), (name,))
	        if cur.fetchone() is None:
		        cur.execute(query2.format('subject'), (name, eng_name))
...
"""
{'full_name': 'dr. Gregor Žvelc', 'office': 'II/07 (Kettejeva 1, Koper)', 'subjects': [('Klinična psihologija', 'Clinical Psychology'), ('Klinično-psihološki intervju', 'Clinical and Psychological Interview'), ('Psihoterapevtski pristopi in psihoterapija', 'Psychoterapeutic Approaches and Psychoterapy'), ('Uvod v klinično psihologijo in psihoterapijo', 'Introduction to Clinical Psychology and Psychotherapy')], 'title_eng': 'Associate Professor', 'departments': [('Oddelek za psihologijo', 'Department of Psychology')], 'title_slo': 'Izredni profesor', 'research': [('Psihoterapija in klinična psihologija', 'Psychotherapy and clinical psychology'), ('Navezanost v odraslosti', 'Adult attachment'), ('Objektni odnosi', 'Object relations'), ('Čuječnost', 'Mindfulness'), ('Integracija psihoterapije', 'Psychotherapy integration')]}
"""
